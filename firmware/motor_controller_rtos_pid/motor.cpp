...
struct WheelPID {
    ...
    void set_gains(const PidConfig &config) {
        kp = config.kp;
        ki = config.ki;
        kd = config.kd;
        reset();
    ...
    void get_gains(PidConfig &config) const {
        config.kp = kp;
        config.ki = ki;
        config.kd = kd;
    }
...
void motor_get_pid_config(PidConfig &config) {
    pid_fl.get_gains(config);
}

void motor_set_pid_config(const PidConfig &config) {
    pid_fl.set_gains(config);
    pid_fr.set_gains(config);
    pid_bl.set_gains(config);
    pid_br.set_gains(config);
}
